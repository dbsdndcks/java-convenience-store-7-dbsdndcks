...
#### 1-5. 멤버십 할인 적용 안내 메시지 출력.
- `멤버십 할인을 받으시겠습니까? (Y/N)`

#### 1-6. 멤버십 할인 적용 여부를 입력 한다.
- 사용자는 Y/N중 하나를 입력 한다.

- Y: 멤버십 할인을 적용한다.
- N: 멤버십 할인을 적용하지 않는다.

#### 1-7. 구매 상품 내역, 증정 상품 내역, 금액 정보를 출력한다
...
### 3️⃣재고 관리
- 입력값에 대해 각 상품의 재고 수량을 고려하여 결제 가능 여부를 확인한다.
- 손님이 상품을 구매하면 해당 상품의 재고에서 수량을 차감한다.
...
### 6️⃣영수증 기능
- 증정 상품, 구매 상품명, 수량, 가격을 다룬다.
- 총구매액에서 총 할인액을 제외한 나머지 값을 구한다.
